...
public class HCSStaffDownloadRegistrationFormUtil
{
    ...
    //ฟังก์ชั่นสำหรับแสดงข้อมูลจากการค้นหาข้อมูลตามเงื่อนไขในส่วนของการขึ้นทะเบียนสิทธิรักษาพยาบาล แล้วส่งค่ากลับเป็น Dictionary<string, object>
    ...
    public class StudentRecordsUtil
    {
        ...
        public static Dictionary<string, object> SetValueDataRecorded(Dictionary<string, object> _dataRecorded, DataSet _ds)
        {
            ...
            //_studentPicture = (!String.IsNullOrEmpty(_studentPicture) && Util.FileSiteExist(_studentPicture) ? (HCSStaffUtil._myHandlerPath + "?action=image2stream&f=" + Util.EncodeToBase64(_studentPicture)) : String.Empty);
    ...
    {
        //ฟังก์ชั่นสำหรับสร้างแบบฟอร์มบริการสุขภาพเป็นไฟล์ PDF
        //โดยมีพารามิเตอร์ดังนี้
        //1. _registrationForm  เป็น string รับค่าชื่อแบบฟอร์มบริการสุขภาพ
        //2. _dataRecorded      เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //3. _e                 เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างแบบฟอร์มหนังสือแสดงเจตนาในการขอเปลี่ยนหน่วยบริการประจำ
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างแบบฟอร์มคำร้องขอลงทะเบียนประกันสุขภาพถ้วนหน้าโรงพยาบาลรามาธิบดี
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างแบบฟอร์มคำร้องขอลงทะเบียนประกันสุขภาพถ้วนหน้าโรงพยาบาลศิริราช
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างแบบฟอร์มคำร้องขอลงทะเบียนประกันสุขภาพถ้วนหน้าตามหน่วยบริการสุขภาพ
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        public static void GetRegisForm(Dictionary<string, object> _dataRecorded, ExportToPDF _e)
        {
            ...
            /*
            _e.FillForm(HCSStaffUtil._myPDFFontNormal, 11, 0, ("วันที่พิมพ์ " + Util.ConvertDateTH(Util.CurrentDate("yyyy/MM/dd"))), 18, 565, 100, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (!_dataRecorded["StudentCode"].ToString().Equals("XXXXXXX") ? Util.GetBlank(_dataRecorded["StudentCode"].ToString(), "") : ""), 192, 572, 91, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["ProgramNameTH"].ToString(), ""), 180, 560, 106, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["FacultyNameTH"].ToString(), ""), 169, 542, 118, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 15, 0, Util.GetFullName(Util.GetBlank(_dataRecorded["TitleInitialsTH"].ToString(), ""), Util.GetBlank(_dataRecorded["TitleFullNameTH"].ToString(), ""), Util.GetBlank(_dataRecorded["FirstName"].ToString(), ""), Util.GetBlank(_dataRecorded["MiddleName"].ToString(), ""), Util.GetBlank(_dataRecorded["LastName"].ToString(), "")), 18, 493, 266, 34);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 1 ? _dataRecorded["IdCard"].ToString().Substring(0, 1) : ""), 23, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 2 ? _dataRecorded["IdCard"].ToString().Substring(1, 1) : ""), 54, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 3 ? _dataRecorded["IdCard"].ToString().Substring(2, 1) : ""), 69, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 4 ? _dataRecorded["IdCard"].ToString().Substring(3, 1) : ""), 84, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 5 ? _dataRecorded["IdCard"].ToString().Substring(4, 1) : ""), 100, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 6 ? _dataRecorded["IdCard"].ToString().Substring(5, 1) : ""), 131, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 7 ? _dataRecorded["IdCard"].ToString().Substring(6, 1) : ""), 147, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 8 ? _dataRecorded["IdCard"].ToString().Substring(7, 1) : ""), 162, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 9 ? _dataRecorded["IdCard"].ToString().Substring(8, 1) : ""), 177, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 10 ? _dataRecorded["IdCard"].ToString().Substring(9, 1) : ""), 193, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 11 ? _dataRecorded["IdCard"].ToString().Substring(10, 1) : ""), 224, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 12 ? _dataRecorded["IdCard"].ToString().Substring(11, 1) : ""), 239, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 16, 0, (Util.GetBlank(_dataRecorded["IdCard"].ToString(), "").Length >= 13 ? _dataRecorded["IdCard"].ToString().Substring(12, 1) : ""), 271, 441, 8, 0);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["BirthDateTH"].ToString(), ""), 72, 428, 87, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["HouseNoPermanentAddress"].ToString(), ""), 56, 397, 48, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["VillageNoPermanentAddress"].ToString(), ""), 123, 397, 27, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["LaneAlleyPermanentAddress"].ToString(), ""), 189, 397, 93, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["RoadPermanentAddress"].ToString(), ""), 33, 382, 90, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["SubDistrictNameTHPermanentAddress"].ToString(), ""), 166, 382, 117, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["DistrictNameTHPermanentAddress"].ToString(), ""), 56, 367, 60, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["ProvinceNameTHPermanentAddress"].ToString(), ""), 141, 367, 60, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["PostalCodePermanentAddress"].ToString(), ""), 238, 367, 45, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["PhoneNumberPermanentAddress"].ToString(), ""), 45, 351, 95, 28);
            _e.FillForm(HCSStaffUtil._myPDFFontBold, 12, 0, Util.GetBlank(_dataRecorded["MobileNumberPermanentAddress"].ToString(), ""), 185, 351, 97, 28);
            ...
            if (!_dataRecorded["OccupationNameTHFather"].ToString().IndexOf("รับราชการ").Equals(-1) || !_dataRecorded["OccupationNameTHFather"].ToString().IndexOf("พนักงาน / ลูกจ้าง ส่วนราชการ").Equals(-1) ||
                ...
                _e.FillForm(HCSStaffUtil._myPDFFontBold, 14, 0, "รอเปลี่ยนสิทธิอายุ 20 ปี", 18, 22, 440, 0);

            if (!String.IsNullOrEmpty(Util.GetBlank(_dataRecorded["IdCard"].ToString(), "")))
                _e.FillForm(HCSStaffUtil._myPDFFontBarcode, 24, 2, "*" + _dataRecorded["IdCard"] + "*", 577, 29, 245, 0);
            */
        }

        //ฟังก์ชั่นสำหรับสร้างแบบลงทะเบียนผู้ป่วยใหม่โรงพยาบาลรามาธิบดี
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างแบบลงทะเบียนผู้ป่วยใหม่โรงพยาบาลศิริราช
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างแบบลงทะเบียนผู้ป่วยใหม่โรงพยาบาลเวชศาสตร์เขตร้อน
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างแบบลงทะเบียนผู้ป่วยใหม่ศูนย์การแพทย์กาญจนาภิเษก
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับสร้างบัตรสุขภาพนักศึกษามหาวิทยาลัยมหิดล
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        public void GetKN001Form(Dictionary<string, object> _dataRecorded, ExportToPDF _e)
        {
            ...
            //_e.FillForm(HCSStaffUtil._myPDFFontBold, 25, 0, "X", 323, 218, 8, 0);
        ...
        //ฟังก์ชั่นสำหรับสร้างแผ่นการรักษาของนักศึกษามหาวิทยาลัยมหิดล
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF
        ...
        //ฟังก์ชั่นสำหรับชื่อและที่อยู่สำหรับจัดส่งเอกสาร
        //โดยมีพารามิเตอร์ดังนี้
        //1. _dataRecorded  เป็น Dictionary<string, object> รับค่าชุดข้อมูล                
        //2. _e             เป็น ExportToPDF รับค่าออบเจ็กของ ExportToPDF

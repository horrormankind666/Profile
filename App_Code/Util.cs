...
namespace NUtil
{
    public class Util
    {
        ...
        public static StringBuilder GetStudentRecordsToStudentCV(string _personId)
        {
            ...
            //_html.AppendLine(_ss.GetStudentRecordsToHTML(_personId));
        ...
        public static void SetCellExcel(ExcelWorksheet _ws, int _maxRowCellRange, int _maxColCellRange, int _maxRowCellHeader, int _maxColCellHeader)
        {
            var _cellRange = _ws.Cells;
            var _cellHeader = _ws.Cells;

            _ws.Cells.Style.Font.Name = "Cordia New";
            _ws.Cells.Style.Font.Size = 14;

            _cellRange = _ws.Cells[1, 1, _maxRowCellRange, _maxColCellRange];
            ...
            _cellRange.Style.Fill.PatternType = ExcelFillStyle.Solid;
            _cellRange.Style.Fill.BackgroundColor.SetColor(ColorTranslator.FromHtml("#FFFFFF"));

            _cellHeader = _ws.Cells[1, 1, _maxRowCellHeader, _maxColCellHeader];
            _cellHeader.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
            _cellHeader.Style.Border.Bottom.Style = ExcelBorderStyle.None;
            _cellHeader.Style.Fill.PatternType = ExcelFillStyle.Solid;
            _cellHeader.Style.Fill.BackgroundColor.SetColor(ColorTranslator.FromHtml("#D9D9D9"));

            _ws.Cells[(_maxRowCellHeader + 1), 1, _maxRowCellRange, _maxColCellRange].Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;
            _ws.Cells[(_maxRowCellHeader + 1), 1, _maxRowCellRange, _maxColCellRange].Style.VerticalAlignment = ExcelVerticalAlignment.Top;
            _ws.View.FreezePanes((_maxRowCellHeader + 1), 1);
        }

        public static void GetListDataToExcel(List<object> _cellContent, DataTable _dt, ExcelWorksheet _ws, int _startRow)
        {
            int _i = _startRow;
            int _j = 0;
            ...
            foreach (DataRow _dr in _dt.Rows)
            {
                ...
                {
                    string _field = ((string[])_c)[1];
                    string _horizontalAlignment = ((string[])_c)[2];
                    string _wrapText = ((string[])_c)[3];
                    var _cell = _ws.Cells[_i, _j];

                    if (!String.IsNullOrEmpty(_field))
                    {
                        if (_horizontalAlignment.Equals("center"))
                            _cell.Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;

                        if (_horizontalAlignment.Equals("right"))
                            _cell.Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;

                        if (_wrapText.Equals("true"))
                            _cell.Style.WrapText = true;
                        
                        _cell.Value = (_dr[_field].ToString().Equals("0") ? _dr[_field].ToString() : Util.GetBlank(_dr[_field].ToString(), ""));
                    }
                        ...
                        if (_level4SeriesId.Count > 0)
                        {
                            ...
                            {                                
                                ...
                                for (_j = 0; _j < _seriesNameTemp.Count; _j++)
                                {
                                    ...
                                    if (_level4SeriesColorByPoint.Equals(false))
                                        _html.AppendFormat("    color: '{0}',", _seriesColorTemp[_j]);

...
var HCSStaffSearch = {
    ...
    resetSearch: function (_param) {
        ...
        if (_param["page"] == Util.tut.pageMasterDataHospitalOfHealthCareServiceMain)
            _this = this.masterdata.hospitalofhealthcareservice;

        if (_param["page"] == Util.tut.pageMasterDataRegistrationFormMain)
            _this = this.masterdata.registrationform;

        if (_param["page"] == Util.tut.pageMasterDataAgencyRegisteredMain)
            _this = this.masterdata.agencyregistered;

        if (_param["page"] == Util.tut.pageDownloadRegistrationFormMain)
            _this = this.downloadregistrationform;

        if (_param["page"] == Util.tut.pageOurServicesHealthInformationMain)
            _this = this.ourservices.healthinformation;

        if (_param["page"] == Util.tut.pageOurServicesStatisticsDownloadHealthCareServiceFormMain)
            _this = this.ourservices.statisticsdownloadhealthcareserviceform;

        if (_param["page"] == Util.tut.pageOurServicesTermServiceHCSConsentRegistrationMain)
            _this = this.ourservices.termservicehcsconsent;

        if (_param["page"] == Util.tut.pageOurServicesTermServiceHCSConsentOOCAMain)
            _this = this.ourservices.termservicehcsconsent;
    ...
    actionSearch: function (_param) {
        ...
        Util.actionSearch({
            ...
            if (_param["pageSearch"] == Util.tut.pageDownloadRegistrationFormMain)
    ...
    downloadregistrationform: {
        ...
        validateSearch: function () {
            ...
            if (Util.comboboxGetValue("#" + this.idSectionSearch + "-registrationform") == "0") {
                _error[_i] = "กรุณาเลือกแบบฟอร์มบริการสุขภาพ;Please select health care service form.;";
                _i++;
            }
    ...
    ourservices: {
        ...
        termservicehcsconsent: {
            ...
            validateSearch: function () {
                ...
                if ($("#" + this.idSectionSearch + "-consentdatestart").val().length > 0 && $("#" + this.idSectionSearch + "-consentdateend").val().length == 0) {
                    _error[_i] = "กรุณาใส่วันที่แสดงความยินยอมให้ครบถ้วน;Please enter date of consent in the form completely.;";
                    _i++;
                }
